...
import { updateProjectCACSection, updateProjectUpdateSection } from '../../../../../../firebase';
...
const SectionCreationModal = (props: IProps) => {
          ...
          let data: string[] = []
            if (image.length > 0 && project.name! !== '') {
              image.forEach((img: any, index: number) => {
                const imageRef = isBanner ? storageRef(storage, `images/projects/${project.name!.split(' ').join('_')}/updates/banners/${img.name}`)
                : storageRef(storage, `images/projects/${project.name!.split(' ').join('_')}/updates/${img.name}`)
    ...
    /*
        ...
        const uploadSection = async () => {
            ...
            await updateProjectUpdateSection(project.id!, section)
            close(); closeOuter();
