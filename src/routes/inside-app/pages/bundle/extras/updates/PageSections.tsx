...
const PageSections = (props: IProps) => {
  ...
  const [sections, setSections] = useState(project.updates || []);
  ...
  const deleteSection = async (index: number) => {
    if (index >= 0) {
      ...
      const sectionsRef = ref(database, `projects/${project.id}/updates/`);
      await get(sectionsRef).then((snap) => {
        snap.forEach((item) => {
            ...
            && item.val().image_url == section.image_url && item.val().video_url == section.video_url) {
                const deleteReference = ref(database, `projects/${project.id}/updates/${item.key}`)
      ...
      const endsource = section.image_url
                .replace(`https://firebasestorage.googleapis.com/v0/b/redrum-finance.appspot.com/o/images%2Fprojects%2F${project.name?.replace(/ /g, '_')}%2Fupdates%2F`, '')
                .split('.jpg')[0].replace(/%2F/g, '/').replace(/%20/, ' ')
        deleteObject(storageRef(storage, `images/projects/${project.name?.replace(/ /,'_')}/updates/${endsource}.jpg`))
